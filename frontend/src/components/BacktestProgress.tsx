import type { BacktestJobStatus } from "../api/client";

type Props = {
  status: BacktestJobStatus | null;
};

export function BacktestProgress({ status }: Props) {
  ...
  const barColor = state === "error" ? "#dc2626" : state === "completed" ? "#16a34a" : "#2563eb";

  return (
    <div className="card" style={{ marginBottom: "1rem" }}>
      <h2 style={{ margin: "0 0 0.75rem", fontSize: "1.05rem" }}>Bulk backtest progress</h2>
      <div
        style={{
          height: 12,
          background: "#e2e8f0",
      ...
      <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: "0.5rem" }}>
        ...
        <span style={{ color: "#64748b", fontSize: "0.9rem" }}>
          {completed_tickers} / {total_tickers} tickers done
